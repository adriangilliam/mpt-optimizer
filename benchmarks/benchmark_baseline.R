################################################################################
# MPT Benchmark Script - Baseline
# Date: 2026-03-13
# Contract: June 2026 3M SOFR futures (CME SRM26, expiry 2026-06-17)
# Treasury yields: Treasury.gov, 2026-03-13
# Convention: put_call='P' = put on futures price = cap on rate (f_c, flag=1)
#             put_call='C' = call on futures price = floor on rate (f_p, flag=0)
#             Follows put-call parity: P_price - C_price = B*(F_rate - K_rate)
################################################################################
...
# ---------------------------------------------------------------------------
# Helper: Bachelier (normal) model option price in rate space
# flag=1 → cap (pays when rate > K):  B*[(F-K)*N(d) + σ√T*φ(d)]
# flag=0 → floor (pays when rate < K): B*[(K-F)*N(-d) + σ√T*φ(d)]
# ---------------------------------------------------------------------------
bachelier_price = function(F, K, T, sig, flag) {
  d = (F - K) / (sig * sqrt(T))
  if (flag == 1) {
    (F - K) * pnorm(d) + sig * sqrt(T) * dnorm(d)   # cap
  } else {
    (K - F) * pnorm(-d) + sig * sqrt(T) * dnorm(d)  # floor
  }
}

# ---------------------------------------------------------------------------
# Helper: solve for implied normal vol (minimise SSE across all options)
# F and strikes in decimal; T_exp in years
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# update_data: synthetic SOFR options for 2026-03-13
#
# P (put on futures price = cap on rate): pays max(rate - K, 0) → flag = 1
# C (call on futures price = floor on rate): pays max(K - rate, 0) → flag = 0
# Put-call parity: P_price - C_price = B*(F_rate - K_rate)
#   → slope of (P-C) ~ K is -B  → B_ols = -coef[2] = B > 0  ✓
# ---------------------------------------------------------------------------
update_data = function() {
  obs_date  = as.Date('2026-03-13')
  expy_date = as.Date('2026-06-17')
  days_exp  = as.numeric(expy_date - obs_date)   # 96
  T_exp     = days_exp / 365
  contract  = 'SRM26'

  F_true   = 0.0360      # forward SOFR rate (decimal)
  sig_true = 0.0100      # normal vol (1%, gives ~40 live options)
  B_true   = exp(-0.0372 * T_exp)

  # Strike grid: 12 basis-point steps, ±3 vol-adjusted sigmas
  ...
  rows = do.call(rbind, lapply(strikes, function(K) {
    cap_p   = B_true * bachelier_price(F_true, K, T_exp, sig_true, 1)  # P (cap)
    floor_p = B_true * bachelier_price(F_true, K, T_exp, sig_true, 0)  # C (floor)
    ...
    data.frame(
      ...
      d_y         = days_exp,
      rate_strike = K,
      settle      = c(pmax(cap_p   + noise[1], 1e-8),   # row for P (cap)
                      pmax(floor_p + noise[2], 1e-8)),   # row for C (floor)
      put_call    = c('P', 'C'),    # P=cap, C=floor (futures-price convention)
      option_flag = c(1L, 0L),     # 1=cap(f_c), 0=floor(f_p)
      stringsAsFactors = FALSE
    )
  }))

  # Drop near-zero prices (deep OTM)
  rows = rows[rows$settle > 1e-6, ]

  # Futures
  futures = data.frame(date = obs_date, contract = contract,
                       rate_settle = F_true, mat = T_exp,
                       stringsAsFactors = FALSE)

  # Treasury yield curve, 2026-03-13 (Treasury.gov)
  treas_rates = data.frame(
    date = obs_date,
    mat  = c(1/12, 2/12, 3/12, 4/12, 6/12, 1, 2, 3, 5, 7, 10, 20, 30),
    rate = c(0.0375, 0.0371, 0.0372, 0.0369, 0.0370,
             0.0366, 0.0373, 0.0374, 0.0387,
             0.0407, 0.0428, 0.0489, 0.0490),
    stringsAsFactors = FALSE
  )

  list(options = rows, futures = futures,
       treas_rates = treas_rates, basis_spread = 0,
       T_exp = T_exp)
}

# ---------------------------------------------------------------------------
# Helper functions from original MPT code
# ---------------------------------------------------------------------------
f_p = function(x, parms)   # floor integrand (flag=0)
  (parms[1]-x) * dbeta(pnorm(x,mean=parms[4],sd=parms[5]),
                        parms[2], (parms[3]-parms[2]+1)) *
  dnorm(x, mean=parms[4], sd=parms[5])

f_c = function(x, parms)   # cap integrand (flag=1)
  (x-parms[1]) * dbeta(pnorm(x,mean=parms[4],sd=parms[5]),
                        parms[2], (parms[3]-parms[2]+1)) *
  dnorm(x, mean=parms[4], sd=parms[5])
...
get_transformparms = function(options, futures, treas_rates, basis_spread, T_exp) {
  ...
  lm.output = data.frame(d_y=dys, date=date, contract=contract,
                          days_to_expy=days_to_expy,
                          F_ols=NA, B_ols=NA, sigma_ols=NA,
                          basis_spread=NA, est_type=NA,
                          yield_curve=NA, option_count=0, option_overlap=0,
                          stringsAsFactors=FALSE)
  ...
  if (x_overlap < 2) {
    ...
    # P=cap, C=floor → P-C = B*(F-K) → slope = -B, intercept = B*F
    x$y               = x$P - x$C
...
get_xmat = function(transformparms, options, k) {
  ...
  s_len  = length(s)
  X      = matrix(0.0, nrow = s_len, ncol = k)
  for (j in 1:k) {
    for (i in 1:s_len) {
      parms = c(s[i], j, k, m, sig)
      if (option[i] == 0L) {
        X[i, j] = suppressMessages(integral(f_p, -Inf, s[i], parms=parms, reltol=0))
      } else {
        X[i, j] = suppressMessages(integral(f_c,  s[i], Inf, parms=parms, reltol=0))
      }
    }
  }
  X
...
# ---------------------------------------------------------------------------
# BASELINE estimate_parms
# ---------------------------------------------------------------------------
estimate_parms_baseline = function() {
  k          = 80
  draws      = 250000
  burn       = 100000
  thin       = 150
  ...
  cat(sprintf('F=%.4f%%, B=%.6f, sigma=%.4f%%\n',
              transformparms$F_ols, transformparms$B_ols, transformparms$sigma_ols))

  cat('Building X matrix...\n')
  t0    = proc.time()
  x_mat = get_xmat(transformparms, new_data$options, k)
  ...
  cat(sprintf('Running MCMC (%d draws, %d burn, thin %d → %d samples)...\n',
              draws, burn, thin, rows))
  t2 = proc.time()
  tmp_bvec = get_bvec_cpp(draws, burn, thin, k, rows,
...
cat('=== MPT BASELINE BENCHMARK (2026-03-13) ===\n')
t_wall = proc.time()
result = estimate_parms_baseline()
